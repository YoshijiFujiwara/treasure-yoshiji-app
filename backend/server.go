...
func (s *Server) Route() *mux.Router {
	...
	// 一つのタグでの検索も可能にしたい
	r.Methods(http.MethodGet).Path("/articles/search_by_tag/{tag_id}").Handler(commonChain.Then(AppHandler{articleController.SearchByTag}))
	...
	// comment
	r.Methods(http.MethodGet).Path("/articles/{id}/comments").Handler(commonChain.Then(AppHandler{commentController.Index}))
	r.Methods(http.MethodPost).Path("/articles/{id}/comments").Handler(authChain.Then(AppHandler{commentController.Create}))
	r.Methods(http.MethodGet).Path("/articles/{id}/comments/{comment_id}").Handler(commonChain.Then(AppHandler{commentController.Show}))
	r.Methods(http.MethodPut).Path("/articles/{id}/comments/{comment_id}").Handler(authChain.Then(AppHandler{commentController.Update}))
	r.Methods(http.MethodDelete).Path("/articles/{id}/comments/{comment_id}").Handler(authChain.Then(AppHandler{commentController.Destroy}))
